...
var createPolygon = function() {
    ...
    for (var n = 0; n < 5; n++) {
        var rp1 = _rotate2d(c, p1, n*(360/5));
        var rp0 = _rotate2d(c, p0, ((n+0.5)*(360/5)));
        clipPolygon.push(rp1);
        clipPolygon.push(rp0);
    }

    clipPolygon.push(p1);
};

var drawPolygon = function(ctx, x, y, radius) {
    ctx.beginPath();
    ctx.moveTo(x+(clipPolygon[0].x*radius),
    y+(clipPolygon[0].y*radius));
    for (var n = 1; n < clipPolygon.length; n++) {
        ctx.lineTo(x+(clipPolygon[n].x*radius),
        y+(clipPolygon[n].y*radius));
    }
};
